    ...
    for name in sorted(os.listdir(text_dir)):
        ...
        if os.path.isdir(path):
            for fname in os.listdir(path):
                c += 1
                if c > NUM_SAMPLE:
                    print('Number of total words: %d', len(count_word))
                    most_words = sorted(list(count_word.items()), \
                            key=lambda x: x[1], reverse=True)[:VOCA_SIZE]
                    return texts, titles, dates, most_words
                ...
                for _word in text_split+title_split:
                ...
                '''
                dates.append(name)
    print('load all texts. number of total words: %d', len(count_word))
    most_words = sorted(list(count_word.items()), key=lambda x: x[1], reverse=True)[:VOCA_SIZE]
    return texts, titles, dates, most_words
