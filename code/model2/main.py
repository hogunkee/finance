...
model_a.add(TimeDistributed(Dense(hidden_dim1, activation = 'relu')))
...
#pool_rnn_a = Lambda(lambda x: K.max(x, axis=1), output_shape = (hidden_dim1,))(processed_a)
...
model_b.add(TimeDistributed(Dense(hidden_dim2, activation = 'relu')))
...
#pool_rnn_b = Lambda(lambda x: K.max(x, axis=1), output_shape = (hidden_dim2,))(processed_b)
